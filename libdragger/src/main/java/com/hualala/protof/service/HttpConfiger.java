...
public class HttpConfiger {
     ...
     * 请求地址
     */
    private String url = "http://jpaccess.baidu.com/api_6";
    /**
     * 上下文对象,用来对sp做存取管理
     */
    ...
    private void initHttp() {
        OkHttpClient.Builder httpBuilder = new OkHttpClient.Builder().connectTimeout(DEFAULT_CONN_TIMEOUT, TimeUnit.SECONDS)//连接超时时间 .readTimeout(DEFAULT_READ_TIMEOUT, TimeUnit.SECONDS)//读取超时时间设置
        ...
        retrofitBuilder = new Retrofit.Builder().client(httpBuilder.build()).baseUrl(url)//设置域名
    ...
    private Request addPublicParameter(Request request) {
        ...
        HttpUrl.Builder builder = request.url().newBuilder().addQueryParameter("timestamp", timestamp);
        //添加公共参数(每个接口都携带的参数，还可以继续add)
        //ELog.e("url = " + builder.build());
        Request newRequest = request.newBuilder().method(request.method(), request.body()).url(builder.build()).build();
        return newRequest;
    }
}
